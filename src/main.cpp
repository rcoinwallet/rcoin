...
{
    int64 nSubsidy = 225.5 * nCoinAge * 33 / (365 * 33 + 8) * CENT;
	if (nHeight > 788400) {
		nSubsidy = 0 * COIN;
	}
    return nSubsidy;
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // RCoin: clean up wallet after disconnecting coinstake
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // RCoin: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
...
string GetWarnings(string strFor)
{
    ...
    // RCoin: if detected unmet upgrade requirement enter safe mode
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // RCoin: send latest proof-of-work block to allow the
